...
- **Автоматический мониторинг задач**: Отслеживание создания, изменения статуса и завершения задач в OpenHands
- **Мгновенные уведомления в Telegram**: Оповещения о всех изменениях в реальном времени
- **Гибкая конфигурация**: Настройка через переменные окружения и Docker
- **Модуль словаря**: Дополнительный модуль для работы с определениями слов
- **Полное тестовое покрытие**: Надежная работа благодаря комплексному тестированию
...
### Получение Telegram токена и Chat ID / Getting Telegram Token and Chat ID
...
- [🏗️ Архитектура и компоненты](#️-архитектура-и-компоненты)
...
- [🎯 Особые примечания для развертывания и поддержки](#-особые-примечания-для-развертывания-и-поддержки)
...
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────┐    ┌─────────────┐    ┌──────────────┐   │
│  │   bot.py    │    │ map_maker.py│    │    Tests     │   │
│  │  (Основной  │    │  (Модуль    │    │  (Тесты)     │   │
│  │   модуль)   │◄──►│  словаря)   │◄──►│              │   │
│  └─────────────┘    └─────────────┘    └──────────────┘   │
│         │                          │                       │
│         ▼                          ▼                       │
...
#### 1. **bot.py** - Основной модуль мониторинга / Main Monitoring Module
**Назначение**: Ядро системы, отвечает за опрос API OpenHands и отправку уведомлений в Telegram.

**Ключевые функции**:
- `send_telegram_message()` - Отправка сообщений в Telegram с механизмом повторных попыток
- `fetch_conversations()` - Получение списка разговоров из API OpenHands
- `poll_and_notify()` - Основной цикл опроса и отправки уведомлений
- `main()` - Инициализация и запуск бота

**Конфигурация**:
- `TELEGRAM_TOKEN` - Токен Telegram бота (обязательно)
- `CHAT_ID` - ID чата для уведомлений (обязательно)
- `OPENHANDS_API_URL` - URL API OpenHands (по умолчанию: `http://host.docker.internal:3000`)
- `POLL_INTERVAL` - Интервал опроса в секундах (зафиксирован на 5 секундах)

#### 2. **map_maker.py** - Модуль словаря / Dictionary Module
**Назначение**: Работа с определениями слов, поддержка стандартного и пользовательских словарей.

**Ключевые функции**:
- `get_definitions(word, custom_dict)` - Получение определений слова
- `STANDARD_DICTIONARY` - Стандартный словарь с предопределенными словами

**Особенности**:
- Регистронезависимый поиск
- Поддержка пользовательских словарей
- Обработка различных типов входных данных
- Возврат копий списков для безопасности данных

#### 3. **Тесты (tests/)** - Модуль тестирования / Testing Module
**Назначение**: Обеспечение качества и надежности кода.

**Структура**:
- `test_map_maker.py` - Тесты для модуля словаря
- Полное покрытие тестами модуля `map_maker.py`
- Тестирование граничных случаев и обработки ошибок

#### 4. **Docker инфраструктура** / Docker Infrastructure
**Назначение**: Контейнеризация приложения для простого развертывания.

**Компоненты**:
- `Dockerfile` - Конфигурация Docker образа
- `docker-compose.yml` - Конфигурация Docker Compose для оркестрации
- Готовые образы для быстрого запуска в любой среде

### 🔄 Взаимодействие компонентов / Component Interaction
1. **Основной поток**:
   - `bot.py` запускает цикл опроса каждые 5 секунд
   - Получает данные из API OpenHands через `fetch_conversations()`
   - Сравнивает с предыдущим состоянием
   - Отправляет уведомления через `send_telegram_message()`

2. **Модуль словаря**:
   - Может использоваться независимо от основного бота
   - Предоставляет API для работы с определениями слов
   - Поддерживает расширение через пользовательские словари

3. **Тестирование**:
   - Автоматическое тестирование при изменении кода
   - Проверка корректности работы всех компонентов
   - Обеспечение обратной совместимости

## ⚙️ Установка и настройка / Installation and Configuration
...
### Пример 1: Базовый запуск с мониторингом / Example 1: Basic Launch with Monitoring
```bash
...
### Модуль `map_maker.py` / `map_maker.py` Module
...
Мы приветствуем вклад в развитие проекта! Вот как вы можете помочь: / We welcome contributions to the project! Here's how you can help:
...
Проект включает полное тестовое покрытие модуля `map_maker.py`: / The project includes full test coverage of the `map_maker.py` module:
...
## 🎯 Особые примечания для развертывания и поддержки

### 🔧 Ключевые моменты для успешного развертывания

#### 1. **Сетевая конфигурация**
- **Docker с host network**: При использовании `network_mode: host` в Docker Compose, убедитесь, что контейнер имеет доступ к локальному хосту
- **API доступность**: Убедитесь, что URL API OpenHands доступен из среды выполнения бота
- **Прокси и файрволы**: При работе в корпоративных сетях проверьте настройки прокси и файрволов

#### 2. **Безопасность токенов**
- **Никогда не коммитьте токены**: Токены Telegram бота должны храниться только в переменных окружения или защищенных хранилищах секретов
- **Ротация токенов**: Регулярно обновляйте токены Telegram бота для повышения безопасности
- **Ограничение прав бота**: Настройте бота с минимальными необходимыми правами в Telegram

#### 3. **Мониторинг и логирование**
- **Логи Docker**: Используйте `docker logs --follow openhands-monitor` для мониторинга в реальном времени
- **Системные логи**: Настройте ротацию логов для предотвращения переполнения диска
- **Метрики здоровья**: Регулярно проверяйте доступность бота через мониторинг процессов

#### 4. **Масштабирование**
- **Множественные экземпляры**: Для мониторинга нескольких проектов запускайте отдельные экземпляры бота
- **Балансировка нагрузки**: При высокой нагрузке рассмотрите использование очередей сообщений
- **Резервирование**: Настройте резервные экземпляры для критически важных мониторинговых задач

#### 5. **Обновления и обслуживание**
- **Регулярные обновления**: Обновляйте зависимости Python для исправления уязвимостей безопасности
- **Тестирование обновлений**: Всегда тестируйте обновления в staging-среде перед развертыванием в production
- **План отката**: Имейте план быстрого отката на предыдущую версию при возникновении проблем

#### 6. **Производительность**
- **Интервал опроса**: Текущий интервал в 5 секунд оптимален для большинства случаев. Для снижения нагрузки увеличьте интервал
- **Обработка ошибок**: Бот автоматически повторяет неудачные попытки отправки сообщений
- **Очистка памяти**: Бот автоматически очищает состояние завершенных разговоров

#### 7. **Интеграция с системами мониторинга**
- **Prometheus/Grafana**: Рассмотрите добавление метрик для интеграции с системами мониторинга
- **Alerting**: Настройте дополнительные каналы оповещений помимо Telegram
- **Дашборды**: Создайте дашборды для визуализации состояния мониторинга

### 🛠️ Рекомендации по поддержке

#### 1. **Документация изменений**
- Ведите журнал изменений (CHANGELOG) для всех версий
- Документируйте breaking changes и миграционные пути
- Обновляйте README при добавлении новых функций

#### 2. **Сообщество и обратная связь**
- Отслеживайте issues на GitHub для выявления распространенных проблем
- Предоставляйте шаблоны для отчетов об ошибках
- Отвечайте на вопросы в разделе FAQ

#### 3. **Резервное копирование**
- Регулярно создавайте резервные копии конфигураций
- Храните историю версий Docker образов
- Сохраняйте логи для анализа инцидентов

#### 4. **Процедуры аварийного восстановления**
1. **Бот не запускается**: Проверьте переменные окружения и доступность зависимостей
2. **Уведомления не отправляются**: Проверьте токен Telegram и доступность API
3. **Высокая загрузка CPU**: Увеличьте интервал опроса или оптимизируйте код
4. **Утечки памяти**: Проверьте очистку состояния и обновите зависимости

### 📋 Чеклист для production развертывания

- [ ] Настроены все необходимые переменные окружения
- [ ] Протестирована отправка уведомлений в Telegram
- [ ] Проверена доступность API OpenHands
- [ ] Настроено логирование и ротация логов
- [ ] Созданы резервные копии конфигурации
- [ ] Настроен мониторинг доступности бота
- [ ] Документированы процедуры обновления и отката
- [ ] Протестирована работа в условиях сбоев сети
- [ ] Настроены оповещения о критических ошибках
