...
[![OpenHands](https://img.shields.io/badge/openhands-platform-green.svg)](https://openhands.dev/)
[![Tests](https://img.shields.io/badge/tests-passing-brightgreen.svg)](tests/)
[![Coverage](https://img.shields.io/badge/coverage-100%25-brightgreen.svg)](tests/)

**English**: Monitoring bot for tracking tasks in the OpenHands platform with Telegram notifications. The project also includes a word definitions module (`map_maker.py`) with full test coverage.

**Русский**: Мониторинговый бот для отслеживания задач в платформе OpenHands с отправкой уведомлений в Telegram. Проект также включает модуль для работы с определениями слов (`map_maker.py`) с полным тестовым покрытием.
...
3. Добавьте бота в чат и отправьте сообщение
4. Получите Chat ID:
   ```bash
...
- **🧪 Полное тестовое покрытие**: 100% покрытие тестами модуля словаря

### 🏗️ Архитектура проекта:
Проект состоит из двух основных модулей:

1. **Основной модуль мониторинга (`bot.py`)**:
   - 🎯 Ядро системы мониторинга задач OpenHands
   - ⚡ Асинхронный опрос API OpenHands
   - 📤 Отправка уведомлений в Telegram
   - 💾 Управление состоянием разговоров
   - 🔄 Механизм повторных попыток при ошибках

2. **Модуль словаря (`map_maker.py`)**:
   - 📖 Работа с определениями слов
   - 🔍 Функция `get_definitions()` для поиска определений
   - 🛠️ Поддержка стандартного и пользовательских словарей
   - ✅ Полное тестовое покрытие (40+ тестов)
   - 🧹 Автоматическая нормализация входных данных

3. **Инфраструктура**:
   - **🧪 Тесты**: Полное покрытие тестами модуля словаря
   - **🐳 Docker**: Готовые конфигурации для контейнеризации
   - **📦 Docker Compose**: Упрощенное развертывание
   - **📄 Документация**: Полная документация API и использования

### 🛠️ Технический стек:
- **Язык программирования**: Python 3.11+
- **Асинхронность**: asyncio для эффективного опроса API
- **HTTP клиент**: httpx для асинхронных HTTP запросов
- **Telegram API**: python-telegram-bot для интеграции с Telegram
- **Обработка ошибок**: tenacity для повторных попыток
- **Контейнеризация**: Docker и Docker Compose
- **Тестирование**: pytest для модульного тестирования
- **Документация**: Markdown с поддержкой GitHub Flavored Markdown

### 🔄 Принципы работы:
1. **Периодический опрос**: Бот регулярно опрашивает API OpenHands для получения списка разговоров
2. **Отслеживание состояния**: Сохраняет предыдущее состояние разговоров для обнаружения изменений
3. **Уведомления**: Отправляет уведомления в Telegram при обнаружении новых разговоров или изменении статуса
4. **Очистка кэша**: Автоматически удаляет устаревшие записи из кэша состояний
5. **Обработка ошибок**: Повторные попытки при сбоях сети или API

### 📊 Поток данных:
```
OpenHands API → Бот (bot.py) → Telegram Уведомления
      ↓
   map_maker.py (словарь определений)
```

### 🎨 Особенности реализации:
- **Асинхронная архитектура**: Эффективное использование ресурсов
- **Конфигурация через окружение**: Безопасное хранение чувствительных данных
- **Модульная структура**: Легкость расширения и поддержки
- **Полное тестирование**: Гарантия качества кода
- **Контейнеризация**: Простота развертывания в любой среде
...
Проект включает полное тестовое покрытие модуля `map_maker.py` с более чем 40 тестами, охватывающими все возможные сценарии использования.

### 📊 Обзор тестового покрытия:
- **✅ 100% покрытие кода**: Все строки кода в `map_maker.py` покрыты тестами
- **🧪 40+ тестовых случаев**: Комплексное тестирование всех функций
- **🔍 Все граничные случаи**: Тестирование исключительных ситуаций
- **⚡ Быстрое выполнение**: Все тесты выполняются за несколько секунд

### 📋 Типы тестов:

#### 1. **Базовые тесты**:
   - Проверка основной функциональности `get_definitions()`
   - Поиск существующих слов в стандартном словаре
   - Возврат правильных определений

#### 2. **Граничные случаи**:
   - Обработка несуществующих слов
   - Пустые строки и строки только из пробелов
   - Очень длинные строки (10,000+ символов)
   - Слова с различными типами пробелов (табуляции, переносы строк)

#### 3. **Unicode и специальные символы**:
   - Русские слова и кириллица
   - Слова с дефисами и подчеркиваниями
   - Электронные адреса как слова
   - Специальные символы

#### 4. **Пользовательские словари**:
   - Работа с кастомными словарями
   - Приоритет пользовательского словаря над стандартным
   - Пустые пользовательские словари
   - Словари с `None` как значением

#### 5. **Обработка ошибок и типов данных**:
   - Нестроковые входные данные (числа, булевы значения, `None`)
   - Словари с нестроковыми ключами
   - Значения не в виде списков в словарях
   - Вложенные структуры в определениях

#### 6. **Производительность и безопасность**:
   - Работа с большими словарями (1000+ записей)
   - Возврат копий списков, а не ссылок
   - Неизменяемость входных данных
   - Эффективность поиска (O(1) сложность)
...
# Текстовый отчет с пропущенными строками
python -m pytest tests/ --cov=map_maker --cov-report=term-missing

# XML отчет (для CI/CD)
python -m pytest tests/ --cov=map_maker --cov-report=xml
```
...
Файл `tests/test_map_maker.py` содержит следующие тестовые классы и методы:

1. **`TestGetDefinitions`** - Основной класс тестов:
   - `test_basic_functionality()` - Базовая функциональность
   - `test_word_not_found()` - Слова не найдены
   - `test_case_insensitivity()` - Регистронезависимость
   - `test_whitespace_handling()` - Обработка пробелов
   - `test_empty_string()` - Пустые строки
   - `test_custom_dictionary()` - Пользовательские словари
   - `test_unicode_and_special_characters()` - Unicode символы
   - `test_none_custom_dict()` - None как словарь
   - `test_dict_with_empty_values()` - Пустые значения
   - `test_multiple_definitions_order()` - Порядок определений
   - `test_numbers_as_words()` - Числа как слова
   - `test_long_words()` - Длинные слова
   - `test_edge_cases()` - Граничные случаи
   - `test_return_type()` - Тип возвращаемого значения
   - `test_immutability_of_input_data()` - Неизменяемость данных
   - `test_large_dictionary_performance()` - Производительность
   - `test_dictionary_with_non_string_keys()` - Нестроковые ключи
   - `test_none_word_input()` - None как слово
   - `test_boolean_word_input()` - Булевы значения
   - `test_dict_with_non_list_values()` - Несписковые значения
   - `test_return_copy_not_reference()` - Возврат копий
   - `test_dict_with_none_values()` - None как значения
   - `test_complex_normalization()` - Сложная нормализация
   - `test_nested_structures_in_definitions()` - Вложенные структуры
   - `test_very_long_strings()` - Очень длинные строки
   - `test_unicode_normalization()` - Нормализация Unicode
...
```bash
docker build -t openhands-monitor-test .
docker run --rm openhands-monitor-test python -m pytest tests/
```
